...
export class FeaturedProjects extends HTMLElement {
  connectedCallback() {
    ...
    this.innerHTML = `
      <section id="featured" class="bg-brand-dark text-white py-20 border-y-4 border-brand-dark">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
            ...
            <p class="text-slate-400 max-w-md text-sm sm:text-base mt-2 md:mt-0">
  ...
  renderProjectCard(project) {
    ...
    return `
      <div class="bg-brand-cardDark rounded-2xl border-3 ${border} p-6 flex flex-col justify-between shadow-brutal-emerald hover:translate-y-[-4px] transition-transform">
        <div>
          <div class="flex items-center justify-between mb-4">
            <span class="px-3 py-1 ${badgeClass} text-xs font-extrabold uppercase font-mono rounded-md border border-white">${project.badge}</span>
            <span class="text-xs font-mono ${accentText}">${project.id}</span>
          </div>
          <h3 class="text-2xl font-extrabold text-white mb-2">${project.title}</h3>
          <p class="text-slate-300 text-sm leading-relaxed mb-4">${project.description}</p>
          <div class="space-y-2 mb-6">
            <div class="text-xs font-mono text-slate-400">Key Features:</div>
        ...
        <div>
          <div class="flex flex-wrap gap-1.5 mb-5 font-mono text-[11px]">
            ${project.tags.map((tag) => `<span class="px-2 py-0.5 bg-slate-700 ${accentText} rounded">${tag}</span>`).join("")}
          </div>
          ${project.codeUrl ? `<a href="${project.codeUrl}" target="_blank" class="w-full text-center block ${badgeClass} font-extrabold py-2.5 rounded-xl border-2 border-white hover:opacity-90 transition-colors text-sm">View on Github →</a>` : ""}
        </div>
      </div>
    `;
  }
